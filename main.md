...
int main(){
    printf("%d", add(1, 2)); /* 3 */
    printf("%g", add(1.0, 2.0)) /* 3.0 */
    a = {1, 2, 3};
    b = {4, 5, 6};
    /* printf("arr", add(a, b, 3)); # needs printing function that I didn't implement*/
    return 0;
}
</code></pre>
</section>
...
data = DataFrame(CSV.load("mydata.csv")) |> DataFrame
stuff = do_stuff(data)
...
using StatsBase
using Revise
data = CSV.load("mydata.csv") |> DataFrame
